...
from homeassistant.components.irman.IrMan import IrMan
import queue
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    def cleanup_irman(event):
        global _irMan
        _irMan.close()

    def prepare_irman(event):
        global _irMan
        global _rxCmdQueue
        _irMan = IrMan(config.get(CONF_FILENAME), _rxCmdQueue)
        hass.bus.listen_once(EVENT_HOMEASSISTANT_STOP, cleanup_irman)

    hass.bus.listen_once(EVENT_HOMEASSISTANT_START, prepare_irman)
