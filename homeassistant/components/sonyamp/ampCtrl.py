...
class AmpCtrl(threading.Thread):
    ...
    def cmdGen(self, data):    
        cmd = '\x02' + chr(len(data))
        for x in data:
            cmd = cmd + chr(x)
        return cmd + chr(self.genCrc(data))
    
    
    def sendRawCommand(self, verbose, data, expectedResp=[]):
        cmdData = self.cmdGen(data)
        if verbose:
            print ":".join("{0:x}".format(ord(c)) for c in cmdData)        
        ...
        for i in range(RETRY_LIMIT):
            ...
            if expectedRespLen != 0:
                ...
                ok = (''.join(chr(c) for c in expectedResp)) == resp
                if self.verbose and not ok:
                    print "invalid responce " + (":".join("{0:x}".format(ord(c)) for c in resp))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-a', '--addr', action='store', default='-1',
                        type=int, help='The address to write to')
    parser.add_argument('-v', '--val', action='store', default='0',
                        type=int, help='The value to write')
